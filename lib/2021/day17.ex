defmodule Year2021.Day17 do
  ...
  defp part1(target), do:
    try_velocity(0, 1000, target, []) |> Enum.map(&Map.get(&1, :highest_point)) |> Enum.max()
  ...
  defp step_within_range(%__MODULE__{
  ...
  defp cont_step(%__MODULE__{
    ...
    case x > max_x || y < min_y do
      true -> :too_far
      _ -> step_within_range(%__MODULE__{ state |
  ...
  defp next_velocity({x, y}), do:
    {x - 1, y - 1}

  ## utils

  defp to_target("target area: " <> area), do:
    area
      |> String.split(", ")
      |> Enum.map(
        fn str ->
