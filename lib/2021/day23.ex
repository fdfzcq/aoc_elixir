defmodule Year2021.Day23 do
  ...
  defp is_final_state(%__MODULE__{position: positions}),
    do:
      Enum.all?(
        positions,
        fn {c, p} ->
  ...
  defp do_step(state) do
    case Map.get(state, :energy) > Map.get(state, :least_energy) do
      true ->
        {Map.get(state, :least_energy), Map.get(state, :mem)}

      false ->
        new_state = move_to_correct_cell(state)

        case is_final_state(new_state) do
          true ->
            {min(Map.get(new_state, :energy), Map.get(new_state, :least_energy)),
             Map.get(new_state, :mem)}

          false ->
            move_one_out_of_cell(new_state)
        end
    end
  end

  defp move_one_out_of_cell(
         %__MODULE__{position: positions, mem: mem, least_energy: l, energy: energy} = state
       ) do
  ...
  defp do_move_one_out_of_cell(%__MODULE__{
         position: positions,
         energy: energy,
         least_energy: energy_acc,
         mem: mem,
         moved: moved
       }) do
    in_cell_amphs =
    ...
    {new_least, new_mem} =
      Enum.reduce(in_cell_amphs, {9_999_999, mem}, fn {c, {i, d}}, {e_acc, mem1} ->
        ...
        |> Enum.reduce(
          ...
          fn pos, {e_acc2, mem2} ->
            do_step(%__MODULE__{
              ...
              position:
                [{c, pos} | Enum.reject(positions, fn m -> m == {c, {i, d}} end)] |> Enum.sort(),
              least_energy: e_acc2,
              moved: moved,
              mem: mem2
            })
    ...
    case new_least do
      ...
      _ ->
        {min(new_least + energy, energy_acc),
         Map.put(new_mem, positions, min(new_least, energy_acc))}
    end
  end
  ...
  defp do_move_to_correct_cell(
         %__MODULE__{position: positions, energy: energy, moved: moved} = state
       ) do
    filtered_positions =
      positions
    ...
    case filtered_positions do
      ...
      _ ->
        {new_positions, new_energy, new_moved} =
          try_move_to_cells(filtered_positions, positions, energy, moved)
  ...
  defp try_move_to_cells([{c, i} | t], positions, energy, moved) do
    case find_reachable_target_cell(c, i, positions) do
      :no_match ->
        try_move_to_cells(t, positions, energy, moved)
  ...
  defp only_contains_valid_amphipods(positions, i),
    do:
      !Enum.any?(
        positions,
        fn {c, p} ->
  ...
  defp input(),
    do: [["C", "D", "D", "D"], ["C", "C", "B", "D"], ["A", "B", "A", "B"], ["B", "A", "C", "A"]]

  defp test_input(),
    do: [["B", "D", "D", "A"], ["C", "C", "B", "D"], ["B", "B", "A", "C"], ["D", "A", "C", "A"]]
end
