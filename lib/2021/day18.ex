defmodule Year2021.Day18 do
  ...
  defp calc_res([{{v1, v2}, level} | t], acc),
    do: calc_res(t, [{3 * v1 + 2 * v2, level - 1} | acc])

  defp calc_res([h | t], acc), do: calc_res(t, [h | acc])

  defp pair_up([], acc), do: Enum.reverse(acc)

  defp pair_up([{v1, l}, {v2, l} | t], acc) when is_number(v1) and is_number(v2),
    do: pair_up(t, [{{v1, v2}, l} | acc])

  defp pair_up([h | t], acc), do: pair_up(t, [h | acc])

  defp add_level(list), do: Enum.map(list, fn {val, level} -> {val, level + 1} end)

  defp reduce_list(list),
    do:
      list
      |> IO.inspect()
      |> explode_list([])
  ...
  defp explode_list([{{val1, val2}, level} | t], acc) do
    case level >= 4 do
      true ->
        reduce_list(
          Enum.reverse(tail(acc)) ++ explode(head(acc), {{val1, val2}, level}, head(t)) ++ tail(t)
        )

      # reduce_list(Enum.reverse(explode(val1, level, acc, :left)) ++ explode(val2, level, t, :right))
      false ->
        explode_list(t, [{{val1, val2}, level} | acc])
    end
  end
  ...
  defp explode(nil, {{_, val2}, level}, {val, l}),
    do: pair({0, level - 1}, {adds(val, val2, :left), l})

  defp explode({val, l}, {{val1, _}, level}, nil),
    do: pair({adds(val, val1, :right), l}, {0, level - 1})

  defp explode({v1, l1}, {{val1, val2}, level}, {v2, l2}) do
    case l1 == level - 1 && is_number(v1) do
      true ->
        [{{v1 + val1, 0}, l1}, {adds(v2, val2, :left), l2}]
  ...
  defp split_list([{{val1, val2}, level} | t], acc) when val1 > 9,
    do: reduce_list(Enum.reverse([{val2, level}, split(val1, level) | acc]) ++ t)

  defp split_list([{{val1, val2}, level} | t], acc) when val2 > 9,
    do: reduce_list(Enum.reverse([split(val2, level), {val1, level} | acc]) ++ t)
  ...
  defp translate(list), do: translate(list, 0, [])
