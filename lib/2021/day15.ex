defmodule Year2021.Day15 do
  ...
  defp part1(map),
    do:
      Graph.a_star(to_graph(map), {0, 0}, destination(map), fn _ ->
        200
      end)
  ...
  def part1_bfs(map) do
    step(
      :queue.in({{0, 0}, 1, 0, MapSet.new([{0, 0}])}, :queue.new()),
      map,
      destination(map),
      :queue.new()
    )
  ...
  defp step(queue, map, destination, acc) do
    ...
    new_acc =
      case coor == destination do
        # this will throw an error and stop the program as expected
        true ->
          IO.inspect(dist)
              ...
              case val do
                1 ->
                  ...
                  |> Enum.reduce(
                    ...
                    fn next_coor, a ->
                      :queue.in(
                        {next_coor, Map.get(map, next_coor), dist + 1,
                         MapSet.put(traversed, coor)},
                        a
                      )
                    end
                  )
  ...
  defp straight_path(map, {_, max_y}),
    do:
      map
  ...
  defp neighbours({x, y}, {max_x, max_y}),
    do:
      [{x + 1, y}, {x - 1, y}, {x, y + 1}, {x, y - 1}]
      |> Enum.filter(fn {a, b} -> a >= 0 && b >= 0 && a <= max_x && b <= max_y end)

  defp destination(map),
    do:
      map
      |> Map.keys()
  ...
  defp to_map({line_str, y}, acc_map),
    do:
      line_str
      |> String.codepoints()
      |> Enum.with_index()
