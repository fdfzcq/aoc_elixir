defmodule Year2021.Day22 do
  ...
  defp find_overlaps(set, {x, y, z}) do
    set
    |> Enum.filter(fn {x1, y1, z1} -> overlaps(x, x1) && overlaps(y, y1) && overlaps(z, z1) end)
  ...
  defp overlap_range({f1, t1}, {f2, t2}) when f1 <= f2 and t1 <= t2, do: {f2, t1}
  defp overlap_range({f1, t1}, {f2, t2}) when f1 <= f2 and t1 >= t2, do: {f2, t2}
  defp overlap_range({f1, t1}, {f2, t2}) when f1 >= f2 and t1 <= t2, do: {f1, t1}
  defp overlap_range({f1, t1}, {f2, t2}) when f1 >= f2 and t1 >= t2, do: {f1, t2}
